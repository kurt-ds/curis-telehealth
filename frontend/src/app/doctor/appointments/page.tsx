...
export default function DoctorAppointments() {
  ...
  const calAppointments = useMemo(
    () => filtered.filter(a => a.date === selectedCalDate),
    [filtered, selectedCalDate],
  );
    ...
    <div className="p-4 md:p-8 max-w-5xl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          ...
          <div className="flex gap-1 bg-slate-100 rounded-xl p-1 mt-3 w-fit">
            <button
              ...
              {calAppointments.map(apt => {
                ...
                return (
                  <div key={apt.id} className="bg-white border border-slate-100 rounded-xl px-5 py-4 shadow-sm flex flex-col sm:flex-row sm:items-center gap-3">
                    <div className="w-9 h-9 rounded-full flex-shrink-0 flex items-center justify-center text-white text-xs font-bold" style={{ backgroundColor: apt.avatarColor }}>
                      {apt.initials}
                    </div>
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      {apt.status === 'upcoming' && (
                        <>
                          <Link href={`/doctor/appointments/${apt.id}`} className="text-xs font-bold text-white bg-teal-600 hover:bg-teal-700 px-3 py-1.5 rounded-lg transition-colors">
                            Join
                          </Link>
                          <button
                            onClick={() => openCancel(apt)}
                            className="text-xs font-bold text-red-500 hover:text-red-700 border border-red-200 hover:border-red-400 hover:bg-red-50 px-3 py-1.5 rounded-lg transition-colors"
                          >
                            Cancel
                          </button>
                        </>
                      ...
                      {apt.status === 'completed' && (
                        <Link href={`/doctor/appointments/${apt.id}`} className="text-xs font-semibold text-slate-400 hover:text-slate-600 px-2 py-1.5 rounded-lg hover:bg-slate-50 transition-all">
                          View
                        </Link>
                      )}
                    </div>
